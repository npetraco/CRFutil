...
#npsls2.info$samp.neglogpseudolik
#neg.log.psls2
#warnings()
...
for(i in 1:length(theta1)) {
  for(j in 1:length(theta2)) {
    ...
    #npll.info      <- neglogpseudolik(par = theta.i, crf = fit, samples = samps, conditional.energy.function.type = "feature", ff = f0, update.crfQ = F)
    #neg.log.psls2[i] <- npsls2.info$samp.neglogpseudolik
