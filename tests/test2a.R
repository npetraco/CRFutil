...
#as.data.frame(as.table(gR.dist.info$state.probs)) #**** Looks like col labels messed up??
#sapply(1:nrow(st.sp), function(xx){gR.dist.info$state.probs[st.sp[xx,1], st.sp[xx,2], st.sp[xx,3], st.sp[xx,4]]})
# Is this OK????
names(dimnames(gR.dist.info$state.probs))
order(names(dimnames(gR.dist.info$state.probs)))
as.data.frame(as.table(gR.dist.info$state.probs))[,c(3,4,2,1,5)]
